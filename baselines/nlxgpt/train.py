...
def evaluate_all(results_pred, results_gt):
    ...
    print('bert: {:.2f}'.format(np.mean(bert)),flush=True)
...
def load_pretrained():
    if args.visual_backbone:
        model_path = 'model/nle_model_11_vis'
    else:
        model_path = 'model/nle_model_11'
    tokenizer_path = 'model/nle_gpt2_tokenizer_0'
    ...
    def __getitem__(self, i):
        ...
        if args.visual_backbone:
            if args.answerer:
                ...
                text_b = proc_ques(sample['corrected_answerer_explanation'])
            else:
                ...
                if sample['emotion_after'] == sample['emotion_before']:
                    ...
                    text_b = proc_ques(sample['corrected_explanation_after'])
        else:
        ...
        answer = [self.tokenizer.bos_token] + self.tokenizer.tokenize(" i feel " + answer)
...
class VQAXEvalDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        text_a += proc_ques(sample['caption2'])
        if args.dialog:
            for ut in sample['corrected_conversation']:
                text_a += proc_ques(ut)
        text_a += proc_ques('what is the emotion')
        if args.visual_backbone:
            if args.answerer:
                answer = sample['answerer_emotion']
                exp = proc_ques(sample['corrected_answerer_explanation'])
            else:
                answer = sample['emotion_after']
                if sample['emotion_after'] == sample['emotion_before']:
        ...
        answer = [self.tokenizer.bos_token] + self.tokenizer.tokenize(" i feel")
        answer_len = len(answer)
        tokens += answer
...
nle_data_train_path = 'data/train_data.pkl'
nle_data_eval_path = 'data/dev_data.pkl'
